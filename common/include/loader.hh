...
// an exception thrown when loading a `Level` from a file.
struct LevelFileLoadException : std::runtime_error {
  LevelFileLoadException(const fs::path &path)
    : std::runtime_error {std::format(
        "failed to load level file {}", path.c_str()
      )},
      path {path} {}
...
public:
  ...
  // load, parse, and validate a level file.
  //
  // its theme will be loaded from 2 search roots: the level file's
  // containing directory, and the current working directory, in that
  // order.
  Level(const fs::path &level_file) {
    assert_throw(
      fs::exists(level_file), LevelFileLoadException {level_file}
    );
    ...
    const level::Level level = json::parse(buf.str());
  }

  // validate a raw `level::Level`, and take in a `Theme`.
  //
